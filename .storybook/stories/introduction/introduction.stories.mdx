...
- This wrapper does not contain the [monaco-editor](https://microsoft.github.io/monaco-editor/) library, it will fetch the library accroding to the `vsPath` property (or `vs-path` attribute) of the component. Thus, different versions of `monaco-editor` can be used. The default `vs-path` value is set to the CDN url but it can be replaced with a custom path.
