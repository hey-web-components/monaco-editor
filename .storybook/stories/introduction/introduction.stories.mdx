import {Meta} from '@storybook/addon-docs';

<Meta title="Introduction/Introduction" />

# Introduction

`@hey-web-components/monaco-editor` is a web component wrapper for [monaco-editor](https://microsoft.github.io/monaco-editor/).

## How to install

### Install with NPM

```bash
npm i @hey-web-components/monaco-editor
```

## How to import

### From HTML

To obtain it directly from the CDN, it can be imported by simply adding a `script` tag like this:

```html
<script
  type="module"
  src="https://unpkg.com/@hey-web-components/monaco-editor"
></script>
```

If it is installed locally, then it can be imported by adding a `script` tag like this:

```html
<script
  type="module"
  src="node_modules/@hey-web-components/monaco-editor/dist/index.js"
></script>
```

### From JavaScript

Assuming it is installed locally by using NPM, then it can be imported like this:

```js
import '@hey-web-components/monaco-editor';
```

This `import` statement would define the custom elements automatically.

The custom elements can also be defined manually. The follwing code shows the available component classes that can be imported.

```js
import {
  HeyMonacoEditor,
  HeyMonacoDiffEditor,
} from '@hey-web-components/monaco-editor';
```

The imported classes can be defined using `customElements.define()` like this:

```js
customElements.define('hey-monaco-editor', HeyMonacoEditor);
customElements.define('hey-monaco-diff-editor', HeyMonacoDiffEditor);
// different tag names could be used here if needed
```

## How to use

For standard editor:

```html
<hey-monaco-editor></hey-monaco-editor>
```

For diff editor:

```html
<hey-monaco-diff-editor></hey-monaco-diff-editor>
```

More details can be found from the "Components" section.

### Notes

- For React, it is recommended to use [@lit-labs/react](https://www.npmjs.com/package/@lit-labs/react) to convert the web elements into React components.
- This wrapper does not contain the [monaco-editor](https://microsoft.github.io/monaco-editor/) library, it will fetch the library accroding to the `vsPath` property (or `vs-path` attribute) of the component. The default `vs-path` value is set to the CDN url but it can be replaced with a custom path.
